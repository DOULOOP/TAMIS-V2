...
export default function AidRouteAnalyzer() {
  ...
  async function fetchOptimalRoute() {
    ...
    try {
      ...
      const res = await fetch(`https://tamis-v2.onrender.com/api/findOptimalRoute?${params.toString()}`, {
  ...
  async function deleteZone(id?: string) {
    if (!id) return;
    try {
      const res = await fetch(`https://tamis-v2.onrender.com/api/closed-zones/${id}`, { method: 'DELETE' });
